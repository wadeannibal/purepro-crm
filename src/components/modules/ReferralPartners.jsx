...
export default function ReferralPartners({ navigateTo }) {
  ...
  const partnerDupes = useMemo(() => {
    ...
    for (const p of partners) {
      ...
      if (name.length >= 4 && pName.length >= 4 && (pName === name || pName.includes(name) || name.includes(pName))) reasons.push('name')
      if (company.length >= 4 && pCompany.length >= 4 && (pCompany === company || pCompany.includes(company) || company.includes(pCompany))) reasons.push('company')
